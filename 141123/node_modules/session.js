...
module.exports={
	read:	function (cookie, fn)
	{
		...
		var session={};
		//读文件
		fs.readFile('session/'+cookie['sessid'], function (err, data){
			if(!err)
			{
				try
				{
	...
	write:	function (cookie, session)
	{
		...
		*/
		//坑——留着
		fs.writeFileSync('session/'+cookie['sessid'], JSON.stringify(session));
	},
		...
		timerAutoRemove=setInterval(function (){
			fs.readdir('session/', function (err, files){
				...
				else
				{
					...
					{
						(function (index){
							fs.stat('session/'+files[i], function (err, stat){
								if(err)
									console.log('清除session错了');
								else
								{
									...
									if(sec>=expires)
									{
										fs.unlink('session/'+files[index], function (err){
											if(err)
												console.log('删除有错');
										});
									}
